...
class IsDriver(permissions.BasePermission):
    def has_permission(self, request, view):
        u = request.user
        return (
            u and u.is_authenticated and u.groups.filter(name__iexact="driver").exists()
        )
...
class DeliveryViewSet(viewsets.GenericViewSet):
    ...
    def _owned_locked(self, request, pk) -> Delivery:
        with transaction.atomic():
            d = get_object_or_404(
                Delivery.objects.select_for_update(), pk=pk, driver=request.user
            )
            return d
    ...
    def deliver(self, request, pk=None):
        ...
        try:
            ...
            d.save(
                update_fields=["status", "picked_up_at", "delivered_at", "updated_at"]
            )
        except ValueError as e:
            return response.Response(
                {"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST
