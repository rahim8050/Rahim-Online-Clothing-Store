...
def aggregate_kpis_daily(org_id: int, for_date: Optional[date] = None) -> VendorKPI:
    ...
    tx_qs = Transaction.objects.filter(vendor_org_id=org_id, processed_at__date__gte=start, processed_at__date__lte=end)
    ...
    # Refunds
    refunds = Refund.objects.filter(transaction__vendor_org_id=org_id, status=Refund.Status.SUCCEEDED, created_at__date__gte=start, created_at__date__lte=end).count()
...
def bump_realtime_on_success(org_id: int, amount: Decimal, net: Decimal):
    key = _rt_key(org_id)
    snap = cache.get(key) or {"gross_revenue": Decimal('0.00'), "net_revenue": Decimal('0.00'), "orders": 0}
...
def bump_realtime_on_refund(org_id: int):
    key = _rt_key(org_id)
    snap = cache.get(key) or {"gross_revenue": Decimal('0.00'), "net_revenue": Decimal('0.00'), "orders": 0, "refunds": 0}
    snap["refunds"] = int(snap.get("refunds", 0)) + 1
    cache.set(key, snap, timeout=3600)


def get_realtime_kpi_snapshot(org_id: int):
    return cache.get(_rt_key(org_id)) or {"gross_revenue": "0.00", "net_revenue": "0.00", "orders": 0, "refunds": 0}
