...
class VendorApplyTests(TestCase):
    ...
    def test_first_apply_success(self):
        ...
        assert VendorApplication.objects.filter(
            user=self.user, status=VendorApplication.PENDING
        ).exists()
    ...
    def test_reapply_after_rejected_requires_kyc(self):
        ...
        VendorApplication.objects.create(
            user=self.user,
            status=VendorApplication.REJECTED,
            company_name="Acme",
            phone="071",
            kra_pin="A123456789B",
            national_id="1",
    ...
    def test_apply_when_approved_blocks(self):
        ...
        VendorApplication.objects.create(
            user=self.user,
            status=VendorApplication.APPROVED,
            company_name="Acme",
            phone="071",
            kra_pin="A123456789B",
            national_id="1",
