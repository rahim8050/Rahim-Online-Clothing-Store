...
def test_vendor_kpis_scoped(client):
    ...
    p1 = Product.objects.create(category=c, owner=owner, name='A', slug='a', price=10, available=True)
    p2 = Product.objects.create(category=c, owner=other, name='B', slug='b', price=20, available=True)

    # One order for owner, one for other
    o1 = Order.objects.create(
        full_name='x', email='e@example.com', address='a', dest_address_text='d', dest_lat=0, dest_lng=0, user=owner
    )
    OrderItem.objects.create(order=o1, product=p1, price=10, quantity=1)
    Transaction.objects.create(user=owner, order=o1, amount=10, method='card', gateway='paystack', status='success', reference='r1')

    o2 = Order.objects.create(
        full_name='x2', email='e2@example.com', address='a2', dest_address_text='d2', dest_lat=0, dest_lng=0, user=other
    )
    OrderItem.objects.create(order=o2, product=p2, price=20, quantity=1)
    Transaction.objects.create(user=other, order=o2, amount=20, method='card', gateway='paystack', status='success', reference='r2')
    ...
    client.logout(); client.force_login(other)
