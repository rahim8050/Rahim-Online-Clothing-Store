...
class VendorKPIAPI(APIView):
  ...
  def get(self, request):
    ...
    orders_30_qs = (Order.objects
                    .filter(vendor_item_q, created_at__gte=since_30)
                    .distinct())
    orders_30d = orders_30_qs.count()

    # Revenue in last 30 days: sum successful transactions for those orders
    rev_map = { t['order_id']: t['amount'] for t in (
      Transaction.objects
        .filter(order__in=orders_30_qs, status='success', created_at__gte=since_30)
        .values('order_id')
    ...
    series_orders = (
      orders_30_qs
      .filter(created_at__gte=since_14)
    ...
    tx_14 = (
      Transaction.objects
        .filter(order__in=orders_30_qs, status='success', created_at__gte=since_14)
        .annotate(d=TruncDate('created_at'))
        .values('d')
        .annotate(amount=Sum('amount'))
    )
    ...
    for i in range(14, 0, -1):
      ...
      k = str(d)
      series_14d.append({ 'date': k, 'orders': orders_by_day.get(k, 0), 'revenue': rev_by_day.get(k, 0.0) })
    ...
    try:
      ...
      last = (Payout.objects
              .filter(vendor_id=owner_id)
              .only('id','amount','created_at')
              .order_by('-created_at')
              .first())
      if last:
        last_payout = { 'amount': float(getattr(last, 'amount', 0) or 0), 'created_at': last.created_at.isoformat() }
    except Exception:
      last_payout = None
