(function () {
  ...
  // Choose destination orientation (as-is vs swapped) closer to driver
  const chooseBestDest = (driver, dest) => {
    if (!driver || driver.lat == null || driver.lng == null || !dest || dest.lat == null || dest.lng == null) return dest;
    const dAsIs = haversineKm(driver, dest);
    const dSwap = haversineKm(driver, { lat: dest.lng, lng: dest.lat });
    return (isFinite(dAsIs) && isFinite(dSwap) && dSwap + 0.1 < dAsIs) ? { lat: dest.lng, lng: dest.lat } : dest;
  };
  ...
  new Vue({
    ...
    methods: {
      ...
      clearPickup(id)    { const d = document.getElementById('pick-date-' + id);  const t = document.getElementById('pick-time-' + id);  if (d) d.value=''; if (t) t.value=''; },
      clearDelivered(id) { const d = document.getElementById('deliv-date-' + id); const t = document.getElementById('deliv-time-' + id); if (d) d.value=''; if (t) t.value=''; },
      ...
      stopTracking() { this._stopGeoWatch(); this._closeWS(); },
      ...
      _ensureWS() {
        ...
        this.ws.onmessage = (e) => {
          ...
          if (msg.type === "position_update") {
          ...
          } else if (msg.type === "status" || msg.type === "status_update") {
            const i = this.deliveries.findIndex(x => x.id === (msg.id || this.activeId));
            if (i >= 0) {
            ...
            if (this.activeId === (msg.id || this.activeId)) this._paintStatic();
          }
        };
      ...
      _closeWS() {
        ...
        this.ws = null; this.wsReady = false; this.outbox = [];
      },
      ...
      async markPickedUp() {
        ...
        const whenISO = (dateEl && timeEl && dateEl.value && timeEl.value) ? this._isoFrom(dateEl.value, timeEl.value) : null;
      ...
      // ---------- Map ----------
      _ensureMap() {
        const containerId = "map-shell";
        let shell = document.getElementById(containerId);
        if (!shell) {
                ...
                <div v-if="activeId===d.id" class="mt-1 grid grid-cols-1 gap-2 text-[11px]">
                  <div class="border rounded-xl p-2">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <button class="px-2 py-1 rounded border" @click="clearPickup(d.id)">Today/Now</button>
                  ...
                  <div class="border rounded-xl p-2">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <button class="px-2 py-1 rounded border" @click="clearDelivered(d.id)">Today/Now</button>
