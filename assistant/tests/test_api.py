...
from orders.models import Delivery, Order, Transaction
...
def test_order_status_by_id_and_order_number(client):
    ...
    r = client.post(
        url,
        data={"session_key": "s1", "message": f"order status {o.id}"},
        content_type="application/json",
    )
    assert r.status_code == 200
    ...
    r = client.post(
        url,
        data={"session_key": "s1", "message": f"order status RAH{o.id}"},
        content_type="application/json",
    )
    assert r.status_code == 200
    assert f"RAH{o.id}" in r.json()["reply"]
...
def test_payment_status_and_delivery_status_paths(client):
    ...
    r = client.post(
        url,
        data={"session_key": "s1", "message": f"payment status {o.id}"},
        content_type="application/json",
    )
    assert r.status_code == 200
    ...
    r = client.post(
        url,
        data={"session_key": "s1", "message": f"delivery status {o.id}"},
        content_type="application/json",
    )
    assert r.status_code == 200
...
def test_help_fallback_when_no_match(client):
    ...
    r = client.post(
        url,
        data={"session_key": "s1", "message": "blabla unknown"},
        content_type="application/json",
    )
    assert r.status_code == 200
