...
class AssistantApiTableTests(TestCase):
    ...
    def test_list_orders_returns_table_structure(self, mock_table):
        ...
        resp = self.client.post('/api/assistant/ask/', data=json.dumps({'session_key':'s1','message':'list orders','persona':'vendor'}), content_type='application/json')
    ...
    def test_fallback_reply_path(self, mock_route):
        ...
        from django.contrib.auth import get_user_model
        u = get_user_model().objects.create_user(username='t2', email='t2@example.com', password='x')
        self.client.force_login(u)
        resp = self.client.post('/api/assistant/ask/', data=json.dumps({'session_key':'s1','message':'hi','persona':'customer'}), content_type='application/json')
        self.assertEqual(resp.status_code, 200)
