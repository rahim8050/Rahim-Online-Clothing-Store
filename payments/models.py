    ...
    class Meta:
        constraints = [
            models.UniqueConstraint(
                fields=["scope", "key"], name="uniq_idem_scope_key"
            ),
        ]
        indexes = [
...
class Transaction(models.Model):
    ...
    status = models.CharField(
        max_length=30, choices=TxnStatus.choices, default=TxnStatus.PENDING
    )
    ...
    def mark_success(self, gateway_reference: str | None = None):
        ...
        self.save(
            update_fields=["status", "gateway_reference", "processed_at", "updated_at"]
        )
        return self
...
class Payout(models.Model):
    ...
    vendor_org = models.ForeignKey(
        VendorOrg, on_delete=models.CASCADE, related_name="payouts"
    )
